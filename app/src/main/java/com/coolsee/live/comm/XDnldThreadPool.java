			...
			case XDNLD_DATATYPE_PICTURE:
				...
				if( null != resultBmp )
				{
					if ( mPicWidth > 0 && mPicHeight > 0)
					{
						...
						scalBmp = Bitmap.createBitmap(resultBmp, 0, 0, resultBmp.getWidth(), resultBmp.getHeight(), matrix,true);
				...
				if ( null == resultBmp && null == scalBmp )
				{
					sendOutputMessage(mHandler, mDataType, XDNLD_MESSAGE_DOWNLOAD_FAILED, mOutArg2);
				...
				else
				{
					if( null == resultBmp )
					{
//						Log.e(TAG, "XDNLD_DATATYPE_PICTURE---> resultBmp is null,"+scalBmp.getWidth());
						sendOutputMessage(mHandler, mDataType, XDNLD_MESSAGE_DOWNLOAD_SUCCESS, mOutArg2, scalBmp);
					}
					else
					{
//						Log.e(TAG, "XDNLD_DATATYPE_PICTURE---> resultBmp is not null"+resultBmp.getWidth());
						sendOutputMessage(mHandler, mDataType, XDNLD_MESSAGE_DOWNLOAD_SUCCESS, mOutArg2, resultBmp);
					}

					//Log.i(TAG, "Download Picture File Success!["+mOutArg2+"]");
					scalBmp = null;
					resultBmp = null;
				}
				break;
...
//			Log.i(TAG, "..... picUrl = " + picUrl);
	...
	public static String downLoadLink(String link, int timeoutConnection, String encode)
    {
		try {
...
//						Log.d(TAG,"error,app_len = "+app_len);
					...
					int bytesRead = 0;
					while((bytesRead = inputStream.read(buff)) > 0 && mExitStream == false) {
